...
export default function Financial() {
  const financialData = mockProducts.map(p => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Acompanhe a lucratividade e o desempenho dos produtos.</p>
      </div>
          ...
          <Card key={p.id} className={i === 0 ? "border-primary bg-primary/5" : ""}>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Trophy className={`h-5 w-5 ${
                i === 0 ? 'text-yellow-500 fill-yellow-500' : 
                i === 1 ? 'text-slate-400 fill-slate-400' : 
                'text-amber-600 fill-amber-600'
              }`} />
            </CardHeader>
            <CardContent>
              <div className="text-xl font-bold truncate" title={p.name}>{p.name}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Análise detalhada de custos, margens e lucratividade total.</CardDescription>
        </CardHeader>
                ...
                {financialData.map(p => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-right">
                      {p.marginValue.toFixed(2)} <span className="text-muted-foreground text-xs">({p.marginPercent.toFixed(1)}%)</span>
                    </TableCell>
